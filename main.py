...
<div class="portfolio-wrapper">
    <div class="black-block">
        ...
        <a href="https://asiertest.streamlit.app/" target="_self" class="back-btn">← Volver al Portfolio</a>
    </div>
</div>
"""
...
# --- PREGUNTA 2 ---
with st.expander("2. Cuéntame algo sobre ti ¿Cómo es Asier Dorronsoro en lo personal?"):
    if ID_VIDEO_2:
        st.markdown(reproductor_youtube(ID_VIDEO_2), unsafe_allow_html=True)
    else:
        st.info("Vídeo 1 en producción... 🚧")

# --- PREGUNTA 3 ---
with st.expander("3. Cuéntame algo sobre ti ¿Cómo es Asier Dorronsoro en lo profesional?"):
    if ID_VIDEO_3:
        st.markdown(reproductor_youtube(ID_VIDEO_3), unsafe_allow_html=True)
    else:
        st.info("Vídeo 2 en producción... 🚧")

# --- PREGUNTA 4 ---
with st.expander("4. El regreso a Euskadi ¿Por qué?"):
    if ID_VIDEO_4:
        st.markdown(reproductor_youtube(ID_VIDEO_4), unsafe_allow_html=True)
    else:
        st.info("Vídeo 3 en producción... 🚧")

# --- PREGUNTA 5 ---
with st.expander("5. ¿Qué te hace único?"):
    if ID_VIDEO_5:
        st.markdown(reproductor_youtube(ID_VIDEO_5), unsafe_allow_html=True)
    else:
        st.info("Vídeo 4 en producción... 🚧")

# --- PREGUNTA 6 ---
with st.expander("6. ¿Qué buscas en tu próximo reto profesional en Euskadi?"):
    if ID_VIDEO_6:
        st.markdown(reproductor_youtube(ID_VIDEO_6), unsafe_allow_html=True)
    else:
        st.info("Vídeo 5 en producción... 🚧")